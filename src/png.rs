...
impl Png {
    ...
    pub fn add_message(&mut self, chunk: Chunk) {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        match index {
            ...
            None => Err("Chunk Type Not found".into()),
        }
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let chunk_bytes: Vec<u8> = self.chunks.iter().flat_map(|i| i.as_bytes()).collect();
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        match file {
            Ok(file) => Self::try_from(file.as_ref()),
            Err(e) => Err(e.into()),
        }
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        let mut chunks_bytes = &value[8..];

        loop {
            let bytes_length = chunks
                .iter()
                .fold(0, |acc, x: &Chunk| acc + x.as_bytes().len());

            if bytes_length + 8 >= value.len() {
                break;
            }

            match Chunk::try_from(chunks_bytes) {
                Ok(chunk) => {
                    let chunk_length = chunk.as_bytes().len();
                    chunks.push(chunk);
                    chunks_bytes = &chunks_bytes[chunk_length..];
                }
                Err(e) => return Err(e),
            }
        }
